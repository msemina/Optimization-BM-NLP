...
! èOÑèPOÉPAMMA èEóATà PEáìãúTATOB  ÅM
...
!
    IF(IT==0)THEN
        WRITE(*,"(/A,I5)")                '     óàCãO àTEPAñàâ                          IT= ',IT
        WRITE(*,"(A,I5)")                 '     óàCãO BõóàCãEHàâ îìHKñàà                NF= ',NF
        SELECT CASE(PODRP)
            CASE(1)
                WRITE(*,"(A,D12.5)")      '     áHAóEHàE îìHKñàà                        F=  ',FX
            CASE(2)
                WRITE(*,"(A,D12.5)")      '     áHAóEHàE îìHKñàà                        F=  ',FX
                WRITE(*,"(A,D12.5)")      '     HOPMA ÉPAÑàEHTA                         NGR=',NGR
            CASE(3)
                WRITE(*,"(A,D12.5)")      '     áHAóEHàE îìHKñàà                        F=  ',FX
                WRITE(*,"(A,D12.5)")      '     HOPMA ÉPAÑàEHTA                         NGR=',NGR
                WRITE(*,"(A,8(D12.5,1X))")'     TEKìôAü TOóKA                           X=  ',(X(I),I=1,N)
        END SELECT
    ELSE
        SELECT CASE(PODRP)
            CASE(1)
