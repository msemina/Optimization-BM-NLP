...
! èEóATú àCXOÑHõX ÑAHHõX
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")    '     MàHàMàáAñàü KBAáàHúûTOHOBCKàM METOÑOM: A84'
        WRITE(*,"(A,I5)")   '     PAáMEPHOCTú èPOCTPAHCTBA èEPEMEHHõX  N= ',N
        WRITE(*,"(A,D12.5)")'     TOóHOCTú PEòEHàü áAÑAóà              E= ',E
        WRITE(*,"(A,I5)")   '     óàCãO àTEPAñàâ                       D= ',D
        WRITE(*,"(A,D12.5)")'     HAóAãúHõâ òAÉ CèìCKA                 ST=',ST
        WRITE(*,"(A,D12.5)")'     MAÜOPAHTA ÉOãÑCTEâHA                 E1=',E1
        WRITE(*,"(A,I5)")   '     HOMEP BEPCàà METOÑA                  V= ',V
        WRITE(*,"(A,I5)")   '     èOPüÑOK ÑàîîEPEHñàPOBAHàü            AC=',PORDIF
        ...
        SELECT CASE(SP_VAR)
            ...
            CASE(300)
                ...
                IF(DABS(C3)<DABS(C0))THEN
                    IF(GRAT)THEN
                        WRITE(*,"(A)")'     OÑHOMEPHõâ èOàCK HEìÑAóEH'
                        SP_VAR=20
                    ELSE
                        GRAT=.TRUE.
                        SP_VAR=100
