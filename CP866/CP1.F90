...
! 
    ...
    END DO
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")            '     METOÑ BOáMOÜHõX HAèPABãEHàâ: CP1'
        ...
        WRITE(*,"(A,D12.5,A,D12.5)")'     èAPAMETPõ BCèOMOÉ. áAÑAóà       B5= ',B5,', E1=',E1
    ...
    DO I=1,N
        IF(X(I)<A(I)-EW.OR.X(I)>B(I)+EW)THEN
           WRITE(*,"(10X,'HAó.TOóKA BHE èAPAããEãEèàèEÑA')")
           RETURN
        END IF
    END DO
    IF(ST1>0.001)THEN
...
! HAóAãO OCHOBHOÉO ñàKãA
    ...
    DO
        ...
        DO
            ...
            NF=DSQRT(NF)
            IF(NF<1.D-3)THEN
                WRITE(*,"(10X,'TOóKA BHìTP. ÉPAÑ. îìHK. <MàH')")
                IF(L1>M)RETURN
                DO I=L1,M
                    P(I)=0
                END DO
            ...
            ELSE
                ...
                IF(FA>=0.999*1.D+15)THEN
                    IF(E1>1.D-8)THEN
                        ...
                        CYCLE
                    END IF
                    WRITE(*,"(10X'HET PEòEHàü áAÑAóà ãè')")
                    RETURN
                END IF
                DO I=1,N
                    HX(I)=ES(I*N2+1)+D1(I)
            ...
            ELSE
                IF(E1<1.D-8)THEN
                    WRITE(*,"(10X,'TOãôàHA CãOü < MàHCãOâ')")
                    RETURN
                END IF
                IF(I1>1)THEN
                    E1=E1/2
