...
! 
    ...
    NS=NS-1
    NGR=DSQRT(NG1)
! èÖóÄíú àëïéÑçõï ÑÄççõï
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")    '     MàHàMàáAñàü METOÑOM COèPüÜEHHõX ÉPAÑàEHTOB: AP3'
        WRITE(*,"(A,I5)")   '     HOMEP BEPCàà METOÑA                     VAR=',VAR
        WRITE(*,"(A,I5)")   '     PAáMEPHOCTú áAÑAóà                      N=  ',N
        WRITE(*,"(A,D12.5)")'     TOóHOCTú PEòEHàü áAÑAóà                 E=  ',E
        WRITE(*,"(A,D12.5)")'     HAóAãúHõâ òAÉ CèìCKA                    C=',C
        WRITE(*,"(A,D12.5)")'     TOóHOCTú OÑHOMEPHOâ MàHàMàáAñàà         E1= ',E1
        WRITE(*,"(A,D12.5)")'     MàHàMAãúHõâ òAÉ CèìCKA                  EV= ',EV
        WRITE(*,"(A,I5)")   '     BOCCTAHOBãEHàE                          M=  ',M
        WRITE(*,"(A,I5)")   '     MAKCàMAãúHOE óàCãO àTEPAñàâ             D=  ',D
        WRITE(*,"(A,I5)")   '     èOPüÑOK ÑàîîEPEHñàPOBAHàü               AA= ',AA
        WRITE(*,"(A,D12.5)")'     òAÉ ÑàîîEPEHñàPOBAHàü                   H=  ',H
    END IF
    CALL PRTUCM(K,NF,N,X,Y,NGR,1,PODRP)
! HAóAãO ÉãABHOÉO ñàKãA
    K=0
    DO
        SELECT CASE(SP_VAR)
            CASE(11111)
        ...
        END SELECT
    END DO
! KOHEñ ÉãABHOÉO ñàKãA
    IF(PODRP/=0)WRITE(*,"(/A)")'     OèTàMAãúHAü TOóKA'
