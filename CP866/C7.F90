...
! 
    ...
    IF(M<=L1)THEN
        DO I=L1,M
            IF(Y(I)>PZ)THEN
                WRITE(*,"(10X,'HAóAãúHAü TOóKA HEÑOèìCTàMA')")
                RETURN
            END IF
        END DO
    END IF
...
! HAóAãO OCHOBHOÉO ñàKãA
    DO
        ...
        CONDP1=COND+1
        IF(CONDP1==COND)WRITE(*,"(/10X,'BõPOÜÑEHHOCTú MATPàñõ')")
...
! 
    ...
    IF(N/=1)THEN
        ...
        DO J=1,N
          ...
          IF (T>ANORM) ANORM=T
        END DO
        ! ÉAìCCOBO àCKãûóEHàE C óACTàóHõM BõÅOPOM BEÑìôEÉO ùãEMEHTA
        DO K=1,NM1
            KP1=K+1
            ! HAâTà BEÑìôàâ ùãEMEHT
            M=K
            DO I=KP1,N
            ...
            A(M,K)=A(K,K)
            A(K,K)=T
            ! èPOèìCTàTú ùTOT òAÉ, ECãà BEÑìôàâ ùãEMEHT PABEH Hìãû
            IF(T/=0.0)THEN
                ! BõóàCãàTú MHOÜàTEãà
                DO I=KP1,N
                    A(I,K)=-A(I,K)/T
                END DO
                ! èEPECTABãüTú à àCKãûóATú èO CTOãÅñAM
                DO J=KP1,N
                    T=A(M,J)
                    A(M,J)=A(K,J)
                    A(K,J)=T
            ...
            END IF
        END DO
        ! COND=(1-HOPMA MATPàñõ A)*(OñEHKA Ñãü 1-HOPMõ MATPàñõ,OÅPATHOâ K A)
        ! OñEHKA èOãìóAETCü èOCPEÑCTBOM OÑHOÉO òAÉA METOÑA OÅPATHõX àTEPAñàâ Ñãü HAàMEHúòEÉO 
        ! CàHÉìãüPHOÉO BEKTOPA. ùTO TPEÅìET PEòEHàü ÑBìXCàCTEM ìPABHEHàâ,
        ! (TPAHCèOHàPOBAHHAü Ñãü A)*Y=E à A*Z=Y,ÉÑEE-BEKTOP àá+1 à -1,
        ! BõÅPAH-Hõâ TAK,óTOÅõ MAKCàMàáàPOBATú BEãàóàHì Y.
        ! ESTIMATE=(1-HOPMA Z)/(1-HOPMA Y)
        ! PEòàTú CàCTEMì (TPAHCèOHàPOBAHHAü Ñãü A)*Y=E
        DO K=1,N
            T=0.0
            IF(K/=1)THEN
                KM1=K-1
