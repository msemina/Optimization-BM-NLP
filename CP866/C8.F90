...
! éèàëÄçàÖ èÖêÖåÖççõï
    ...
    LOGICAL::POVTOR
    EXTERNAL F,CGR
! èPàCBOEHàE áHAóEHàâ KOHCTAHTAM,èAPAMETPAM METOÑA à àX AHAãàá
    CONST1=1.D-60
    CONST2=1.D-15
    E=PAR(1)
    KK=PAR(3)
    ...
    N2=N
    N3=N2-N1+1
! BõÑEãEHàE AKTàBHõX OÉPAHàóEHàâ
    POVTOR=.FALSE.
    11111 CONTINUE
    CALL F(X,ZNFUNK,-1)
    MACTIV=L
    ...
    END IF
    N4=N3+MACTIV
! OCHOBHõE ÑEâCTBàü METOÑA HúûTOHA áAÑAHàE áHAóEHàâ  XPR,DVOY
    N11=N1-1
    IF(N11/=0)THEN
        DO I=1,N11
            XPR(I)=X(I)
    ...
    TEXT=0
    IF(PODRP/=0)THEN
        WRITE(*,"(/5X,'METOÑ HúûTOHA : C8')")
        WRITE(*,"(/5X,'TOóHOCTú PEòEHàü áAÑAóà Hãè',6X,'E=',D12.5)")E
        WRITE(*,"(/5X,'óàCãO àTEPAñàâ',19X,'D=',I5)")D
        WRITE(*,"(/5X,'MAÜOPAHTA ÉOãÑCTEâHA',13X,'MG=',D12.5)")MAJGOS
        WRITE(*,"(/5X,'èAPAMETP Ñãü BõÑEã.AKTàBHõX OÉP.',1X,'SLOY=',D12.5)")SLOY
        WRITE(*,"(/5X,'òAÉ ÑàîîEPEHñàPOBAHàü',12X,'SHAGD=',D12.5)")SHAGD
        WRITE(*,"(/5X,'èOPüÑOK ÑàîîEPEHñàPOBAHàü',8X,'AA=',I5)")AA
        WRITE(*,"(/5X,'òAÉ BõóàCãEHàü ÉECCàAHA',10X,'SHAGG=',D12.5)")SHAGG
        WRITE(*,"(/5X,'èOPüÑOK BõóàCãEHàü ÉECCàAHA',6X,'AC=',I5)")AC
    END IF
...
! OCHOBHOâ ñàKã
    33333 CONTINUE
        ...
        ! BõóàCãEHàE à OÅPAôEHàE ÉECCàAHA LZZ
        IF(MACTIV>=1)THEN
            DO I=1,MACTIV
                DO I1=1,MACTIV
        ...
        IF(DET==0)THEN
            WRITE(*,"(/10X,'ÉECCàAH BõPOÜÑEH')")
            RETURN
        END IF
        ! BõóàCãEHàE  HúûTOHOBCKOÉO  HAèPABãEHàü
        DO I=1,N4
            NAPR(I)=0
            DO I1=1,N4
            ...
            END DO
        END DO
        ! PEÉìãàPOBKA ÑãàHõ òAÉA BÑOãú HAèPABãEHàü
        STEP=1
        DO
            IF(N1<=N2)THEN
                DO I=N1,N2
            ...
            STEP=STEP/2
        END DO
        ! èPOBEPàM,OÅHìãàãACú ãà KAKAü-ãàÅO ÑBOâCTBEHHAü èEPEMEHHAü èPà HAPìòEHHOM AKTàBHOM OÉPAHàóEHàà
        IF(L1<=MACTIV)THEN
            DO I=L1,MACTIV
                I1=ACTIV(I)
                IF(ZNFPR(I1)>0.AND.DVPR(I)<DSQRT(CONST1))THEN
                    ...
                    ELSE
                        P(I1)=10*NACHDV
                        WRITE(*,"(/10X,'METOÑ CXOÑàTCü HE K PEòEHàû')")
                    END IF
                    RETURN
                END IF
            END DO
        END IF
        ! èPOBEPàM, ÑOÅABàãàCú ãà HOBõE AKTàBHõE OÉPAHàóEHàü
        IF(L<=MACTIV)THEN
            L11=L
            IF(L==0) L11=1
            DO I=L11,MACTIV
            ...
            END DO
        END IF
! èEPEKAóKA àá MACCàBOB: XPR,DVPR,ZNFPR,NORLZ1 B MACCàBõ: X,DVOY,P,ZNFUNK,NORLZ.
        DO I=N1,N2
            X(I)=XPR(I)
        END DO
        IF(L/=0)THEN
...
! KOHEñ ñàKãA
    ...
    IF(PODRP/=0)WRITE(*,"(/10X,'OèTàMAãúHAü TOóKA')")
    CALL PRTNLP(KK,N,L,M,X,ZNFUNK,P,ST,CRIT,1,4,TEXT)
    RETURN
END SUBROUTINE C8
!-------------------------------------------------------------------------------------------------------------------------------
! BõóàCãEHàE ÉPAÑàEHTA LZ à EÉO HOPMõ, óACTàóHOE áAèOãHEHàE MATPàñõ  LZZ .
