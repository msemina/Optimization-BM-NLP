! ÑÇéâëíÇÖççõâ åÖíéÑ åéÑàîàñàêéÇÄççéâ îìçäñàà ãÄÉêÄçÜÄ
...
! 
    ...
    END DO
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")    '     ÑBOâCTBEHHõâ METOÑ MOÑàîàñàPOBAHHOâ îìHKñàà ãAÉPAHÜA: CP5'
        ...
        WRITE(*,"(A,D12.5)")'     èAPAMETP PEÉìãüPàáAñàà          R=  ',R
        WRITE(*,"(A,D12.5)")'     èAPAMETP BCèOMOÉ. áAÑAóà        ES= ',ES
        WRITE(*,"(A,D12.5)")'     BECOBOâ KOùîîàñàEHT             R1= ',R1
    END IF
    CALL PRTNLP(0,N,L,M,X,Y,P,ST,CRIT,1,4,TEXT)
    DO I=1,N
        IF(X(I)<A(I)-EW.OR.X(I)>B(I)+EW)THEN
            WRITE(*,"(/10X,'HAóAãúHAü TOóKA BHE èAPAããEãEèàèEÑA')")
    ...
    IF(L1<=M)THEN
        DO I=L1,M
            IF(P(I)<-EW)THEN
                WRITE(*,"(/10X,'ÑBOâCTBEHõâ BEKTOP HEÑOèìCTàM')")
                IF(PODRP/=0)WRITE(*,"(/32X,'OèTàMAãúHAü TOóKA')")
                PAR(1+Q)=EBM
                CALL PRTNLP(KK,N,L,M,X,Y,P,ST,CRIT,1,4,TEXT)
                RETURN
