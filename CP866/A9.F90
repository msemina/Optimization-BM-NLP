...
C-------------
    ...
    2 CONTINUE
      ...
      PRINT 500,NS
  500 FORMAT(/5X,'MàHàMàáAñàü METOÑOM ùããàèCOàÑOB. BEPCàü ',I1)
      PRINT 501,E,D,C,EC,S,AC,H
  501    FORMAT(/5X,'TOóHOCTú PEòEHàü áAÑAóà',17X,'E=',D11.4,
     &       /5X,'óàCãO àTEPAñàâ',26X,'D=',I4,
     &       /5X,'PAÑàìC HAóAãúHOÉO òAPA',18X,'R=',D11.4,
     &       /5X,'TOóHOCTú OÑHOMEPHOâ MàHàMàáAñàà',9X,'EC=',D11.4,
     &       /5X,'MàHàMAãúHõâ OÅ''EM ùããàèCOàÑA',12X,'S=',D11.4,
     &       /5X,'èOPüÑOK ÑàîîEPEHñàPOBAHàü',15X,'P=',I1,
     &       /5X,'òAÉ ÑàîîEPEHñàPOBAHàü',19X,'H=',D11.4)
...
C-------------
    ...
    5 CONTINUE
C
C         BõóàCãEHàE BEKTOPA PACTüÜEHàü
C
      DO 10 I=1,N
         G2(I)=0.D0
        DO 11 J=1,N
...
C
   ...
   15 CONTINUE
C
C         èPOBEPKA BõèOãHEHàü KPàTEPàEB à èEóATú
C
C-------------
      U=F(XN,FNLP)
C-------------
...
10002 IF (V.LT.S) PRINT 200
  200 FORMAT(15X,'OÅúEM ùããàèCOàÑA MAã')
      IF(PODRP.EQ.0) GO TO 10003
      IF (PODRP.GT.0.AND.NG.LT.E) PRINT 201
  201 FORMAT(15X,'ÑOCTàÉHìTA áAÑAHHAü TOóHOCTú')
      IF (PODRP.GT.0.AND.KK.EQ.D) PRINT 202
  202 FORMAT(15X,'BõèOãHEHO áAÑAHHOE óàCãO òAÉOB')
