...
! E—ATœ ˆCXO„H›X „AHH›X
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")    '     MˆHˆMˆ‡A–ˆŸ METO„OM PŸMOƒO OˆCKA: AP4'
        WRITE(*,"(A,D12.5)")'     TO—HOCTœ PE˜EHˆŸ ‡A„A—ˆ             E= ',E
        WRITE(*,"(A,I5)")   '     —ˆC‹O ˆTEPA–ˆ‰                      D= ',D
        WRITE(*,"(A,D12.5)")'     HA—A‹œH›‰ ˜Aƒ C“CKA                C= ',C
        WRITE(*,"(A,D12.5)")'     KO””ˆ–ˆEHT “MEHœ˜EHˆŸ ˜AƒA         C1=',C1
        WRITE(*,"(A,D12.5)")'     TO—HOCTœ B›O‹HEHˆŸ OƒPAHˆ—EHˆ‰     TG=',TG
        WRITE(*,"(A,I5)")   '     —ˆC‹O OTˆMˆ‡ˆP“EM›X APAMETPOB     N= ',N
    END IF 
    CALL PRTUCM(K,NF,N,X,Y,0,1,PODRP)
    K=1
    KK=1
! HA—A‹O OCHOBHOƒO –ˆK‹A. OˆCK O KOOP„ˆHATH›M HAPAB‹EHˆŸM
    DO
        DO I=1,N
            P=X2(I)
            IF(X2(I)<(B(I)-TG))THEN
        ...
        END DO
        IF(Y<=T)THEN
            ! OˆCK HE“„A—EH BAPˆAHT› A B
            IF(L<=1)THEN
                ! B ˆ‡ TEK“™EƒO MˆHˆM“MA
                P=0
                DO I=1,N
                    P=P+ST(I)
                    ST(I)=ST(I)*C1
            ...
            CYCLE
        END IF
        ! “CE˜H›‰ OˆCK. ˆTEPA–ˆŸ ‡ABEP˜AETCŸ
        Y=T
        DO I=1,N
             X(I)=X2(I)
        END DO
        ...
        K=K+1
        KK=K
        ! HA—A‹O HOBO‰ ˆTEPA–ˆˆ TO—KA KCTPAO‹Ÿ–ˆˆ
        DO I=1,N
            X2(I)=2*X(I)-X1(I)
            IF(X2(I)>B(I)) X2(I)=B(I)
            IF(X2(I)<A(I)) X2(I)=A(I)
        END DO
        ! BCOMOƒATE‹œH›E OEPA–ˆˆ
        IF(L>1)THEN
            DO I=1,N
                X1(I)=X(I)
            END DO
        ...
        T=R1
    END DO
! KOHE– OˆCKA O KOOP„ˆHATH›M HAPAB‹EHˆŸM
