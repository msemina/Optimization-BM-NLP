...
! œE◊AT‹ »CXOƒH€X ƒAHH€X
    IF(PODRP/=0)THEN
        WRITE(*,"(5X,'M»H»M»«A÷»ﬂ KBA«»H‹ﬁTOHOBCK»M METOƒOM')")
        ...
        WRITE(*,"(5X,'HOMEP BEPC»» METOƒA',18X,               'V=',I3)")V
        WRITE(*,"(5X,'œOPﬂƒOK ƒ»‘‘EPEH÷»POBAH»ﬂ',12X,         'AC=',I3)")PORDIF
        ...
        IF(SP_VAR==300)THEN
            DO
                ...
                IF(DABS(C3)<DABS(C0))THEN
                    IF(GRAT)THEN
                        WRITE(*,"(5X,'OƒHOMEPH€… œO»CK HE”ƒA◊EH')")
                        EXIT
                    END IF
                    GRAT=.TRUE.
                    SP_VAR=100
