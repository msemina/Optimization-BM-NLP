...
! йнд опнцпюллш
    ...
    CRIT=ST
    IF(PODRP/=0)THEN
        WRITE(*,"(/26X,'METOд OTшCKAHхъ дOосCTхMшX TOвEK: BHYTP')")  
        WRITE(*,"(/5X,'TOвHOCTэ PEьEHхъ гAдAвх Hко',5X,'E=',D12.5,/5X,'вхCкO хTEPAжхи',18X,'D=',I5)")E,D
    END IF
    CALL PRTNLP(KK,N,L,M,X,Y,P,ST,CRIT,1,4,TEXT)
    DO I=1,N
        IF(X(I)<A(I)-EW.OR.X(I)>B(I)+EW)THEN
            WRITE(*,"(10X,'HAвAкэHAъ TOвKA BHE оAPAккEкEохоEдA')")
            RETURN
        END IF
    END DO
    IF(CRIT<E)THEN
        ...
        RETURN
    END IF
! HAвAкO OCHOBHOцO жхKкA
    DO
        K=KK+1
        IF(.NOT.(K<D+1.AND.DABS(CRIT)>E))EXIT
        E1=E1/K; PAR(1+Q)=E1; BB=E*0.2/(1.+M)/K**2
        CALL UNCONS(N,X,A,B,FH,AGR,AGES,YS,G,Q,PAR,F) ! аегялшякеммши бшгнб "осярни" ондопнцпюллш
        KKK=PAR(3+Q)
        IF(KKK==0)THEN
            E1=E1*0.2
            PAR(1+Q)=E1
