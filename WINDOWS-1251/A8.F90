...
! Œœ»—¿Õ»≈ œ≈–≈Ã≈ÕÕ€’
    ...
    LOGICAL::GRAT
    ! ¬¬≈ƒ®Ã ƒŒœŒÀÕ»“≈À‹Õ”ﬁ œ≈–≈Ã≈ÕÕ”ﬁ ƒÀﬂ «¿Ã≈Õ€ ¡≈«”—ÀŒ¬Õ€’ œ≈–≈’ŒƒŒ¬
    INTEGER::TMP
    EXTERNAL F,FNLP
!-------------------------------------------------------------------------------------------------------------------
    E=PAR(1+Q)
...
! œE◊AT‹ »CXOƒH€X ƒAHH€X
    IF(PODRP/=0)THEN
        WRITE(*,"(/5X,'M»H»M»«A÷»ﬂ METOƒOM H‹ﬁTOHA.BEPC»ﬂ 2.')")
        WRITE(*,"(/5X,'PA«MEPHOCT‹ œPOCTPAHCTBA œEPEMEHH€X',2X,'N=',I3)")N
        WRITE(*,"(/5X,'TO◊HOCT‹ PEÿEH»ﬂ «AƒA◊»',14X,'E=',D11.4)")E
        WRITE(*,"(/5X,'◊»CÀO »TEPA÷»…',23X,'D=',I5)")D
        WRITE(*,"(/5X,'HA◊AÀ‹H€… ÿA√ Cœ”CKA',17X,'ST=',D11.4)")ST
        WRITE(*,"(/5X,'MA∆OPAHTA √OÀƒCTE…HA',17X,'E1=',D11.4)")E1
        WRITE(*,"(/5X,'ÿA√ ƒ»‘‘EPEH÷»POBAH»ﬂ',17X,'H=',D11.4,/)")H
    END IF
        ...
        IF(TMP==3)THEN
            DO
                ...
                IF((DABS(C3)<DABS(C0)))THEN
                    IF(GRAT)THEN
                        WRITE(*,"(10X,'OƒHOMEPH€… œO»CK HE”ƒA◊EH')")
                        RETURN
                    ELSE
                        GRAT=.TRUE.
                        TMP=1
