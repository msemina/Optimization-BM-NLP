...
C
C
C ÏPÈCBOEHÈE ÇHA×EHÈÉ KOHCTAHTAM,ÏAPAMETPAM METOÄA
C                È ÈX AHAËÈÇ.
C
      CONST1=1.D-60
      CONST2=1.D-15
      ...
      N3=N2-N1+1
C
C  BÛÄEËEHÈE  AKTÈBHÛX  OÃPAHÈ×EHÈÉ
C
      POVTOR=.FALSE.
11111 CALL F(X,ZNFUNK,-1)
 ...
 3    N4=N3+MACTIV
C
C   OCHOBHÛE ÄEÉCTBÈß METOÄA HÜÞTOHA
C   ÇAÄAHÈE ÇHA×EHÈÉ  XPR,DVOY
C
      N11=N1-1
 ...
 77   CONTINUE
      ...
      PRINT 222
  222 FORMAT(/46X,'METOÄ HÜÞTOHA : C8')
      PRINT 555,E,D,MAJGOS,SLOY,SHAGD,AA,SHAGG,AC
  555 FORMAT(/5X,'TO×HOCTÜ PEØEHÈß ÇAÄA×È HËÏ',6X,'E=',D12.5,
     &       /5X,'×ÈCËO ÈTEPAÖÈÉ',19X,'D=',I5,
     &       /5X,'MAÆOPAHTA ÃOËÄCTEÉHA',13X,'MG=',D12.5,
     &       /5X,'ÏAPAMETP ÄËß BÛÄEË.AKTÈBHÛX OÃP.',1X,'SLOY=',D12.5,
     &       /5X,'ØAÃ ÄÈÔÔEPEHÖÈPOBAHÈß',12X,'SHAGD=',D12.5,
     &       /5X,'ÏOPßÄOK ÄÈÔÔEPEHÖÈPOBAHÈß',8X,'AA=',I5,
     &       /5X,'ØAÃ BÛ×ÈCËEHÈß ÃECCÈAHA',10X,'SHAGG=',D12.5,
     &       /5X,'ÏOPßÄOK BÛ×ÈCËEHÈß ÃECCÈAHA',6X,'AC=',I5)
  333 IF(.NOT.(POVTOR))
     &               CALL PRTNLP(0,N,L,M,X,ZNFUNK,P,ST,CRIT,1,4,TEXT)
C
C  O C H O B H O É    Ö È K Ë    Ï O    K
C
33333 K=KK+1
      IF(.NOT.(K.LT.D+1.AND.DABS(CRIT).GT.E)) GO TO 22222
C
C  BÛ×ÈCËEHÈE È OÁPAÙEHÈE ÃECCÈAHA LZZ
C
 ...
 131  CALL DMINV(LZZ,N4,DET,LRAB,MRAB)
      IF(DET.NE.0) GO TO 161
         PRINT 162
 162     FORMAT(/10X,'ÃECCÈAH BÛPOÆÄEH')
         RETURN
C
C  BÛ×ÈCËEHÈE  HÜÞTOHOBCKOÃO  HAÏPABËEHÈß
C
 161  DO 16 I=1,N4
         NAPR(I)=0
         DO 17 I1=1,N4
 ...
 16   CONTINUE
C
C  PEÃÓËÈPOBKA ÄËÈHÛ ØAÃA BÄOËÜ HAÏPABËEHÈß
C
      STEP=1
 ...
 211  CALL F(XPR,ZNFPR,-1)
     ...
     &                    GO TO 19
      ...
      GO TO 18
C
C  ÏPOBEPÈM,OÁHÓËÈËACÜ ËÈ KAKAß-ËÈÁO ÄBOÉCTBEHHAß ÏEPEMEHHAß
C           ÏPÈ HAPÓØEHHOM AKTÈBHOM OÃPAHÈ×EHÈÈ
C
 ...
 23         P(I1)=10*NACHDV
 25         PRINT 26
 26         FORMAT(/10X,'METOÄ CXOÄÈTCß HE K PEØEHÈÞ')
            RETURN
 22      CONTINUE
C
C  ÏPOBEPÈM, ÄOÁABÈËÈCÜ ËÈ HOBÛE AKTÈBHÛE OÃPAHÈ×EHÈß
C
 ...
 27   CONTINUE
C
C  ÏEPEKA×KA ÈÇ MACCÈBOB: XPR,DVPR,ZNFPR,NORLZ1 B MACCÈBÛ:
...
C
 ...
 9    CONTINUE
      ...
      END
C
C  B Û × È C Ë E H È E    Ø T P A Ô A
C
      SUBROUTINE SHTRAF
