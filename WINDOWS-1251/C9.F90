...
! 
    ...
    END IF  
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")    '     METOƒ ›ÀÀ»œCO»ƒOB: C9'
        WRITE(*,"(A,D12.5)")'     TO◊HOCT‹ PEÿEH»ﬂ «AƒA◊» HÀœ     E= ',E
        WRITE(*,"(A,I5)")   '     ◊»CÀO »TEPA÷»…                  D= ',D
        WRITE(*,"(A,D12.5)")'     PAƒ»”C HA◊AÀ‹HO√O ÿAPA          C= ',C
        WRITE(*,"(A,D12.5)")'     BEC ÿTPA‘A                      EC=',EC
        WRITE(*,"(A,D12.5)")'     œAPAMETP œPEO¡P.                EE=',EE
...
! HA◊AÀO OCHOBHO√O ÷»KÀA
    ...
    DO
        ...
        IF(SP_VAR==0.OR.SP_VAR==1)THEN
            ...
            DO I=1,N
                ...
                END DO
            END DO
            ! œPOBEPKA B€œOÀHEH»ﬂ KP»TEP»EB » œE◊AT‹
            CALL F(XN,Y1,-1)
            CALL STRFUN(L,M,Y1)
            B3=B2*B3
            B4=(B1+1)*B4
    ...
    IF(SP_VAR/=2)THEN
        SIX=6
        WRITE(*,"(10X,'œPEƒ.◊»CÀO »TEPA÷»… ¡E« ”À”◊ÿEH»ﬂ')")
    END IF
! KOHE÷ OCHOBHO√O ÷»KÀA
...
END SUBROUTINE C9
!-------------------------------------------------------------------------------------------------------------------------------
! œPO÷Eƒ”PA B€◊»CÀEH»ﬂ ÿTPA‘A » MAKC»MAÀ‹HO√O O√PAH»◊EH»ﬂ
