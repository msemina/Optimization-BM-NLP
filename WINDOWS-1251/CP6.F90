...
! 
    ...
    DO I=1,N
        IF(X(I)<A(I)-EW.OR.X(I)>B(I)+EW)THEN
            WRITE(*,"(20X,'HA×AËÜHAß TO×KA BHE ÏAPAËËEËEÏÈÏEÄA')")
            RETURN
        END IF
    END DO
    L1=L+1
    IF(L1<=M)THEN
        DO I=L1,M
            IF(P(I)<-EW)THEN
                WRITE(*,"(20X,'ÄBOÉCTBEHHÛÉ BEKTOP HEÄOÏÓCTÈM')")
                RETURN
            END IF
        END DO
    END IF
! ÏOÄÃOTOBKA PAÁO×ÈX MACCÈBOB ÄËß Ï/Ï  FI , DFI
    DO I=1,N
        AA(I)=A(I)
        BB(I)=B(I)
    END DO
...
! HA×AËO OCHOBHOÃO ÖÈKËA
    DO
        ...
        IF(L1<=M)THEN
            ...
            END DO
        END IF
        ! ÏOÄÃOTOBKA PAÁO×ÈX MACCÈBOB ÄËß Ï/Ï FHCP6, AGRCP6
        DO I=1,N
            XX(I)=X(I)
        END DO
        DO I=1,M1
