...
C-------------
    ...
    2 CONTINUE
      ...
      PRINT 500,NS
  500 FORMAT(/5X,'MÈHÈMÈÇAÖÈß METOÄOM ÝËËÈÏCOÈÄOB. BEPCÈß ',I1)
      PRINT 501,E,D,C,EC,S,AC,H
  501    FORMAT(/5X,'TO×HOCTÜ PEØEHÈß ÇAÄA×È',17X,'E=',D11.4,
     &       /5X,'×ÈCËO ÈTEPAÖÈÉ',26X,'D=',I4,
     &       /5X,'PAÄÈÓC HA×AËÜHOÃO ØAPA',18X,'R=',D11.4,
     &       /5X,'TO×HOCTÜ OÄHOMEPHOÉ MÈHÈMÈÇAÖÈÈ',9X,'EC=',D11.4,
     &       /5X,'MÈHÈMAËÜHÛÉ OÁ''EM ÝËËÈÏCOÈÄA',12X,'S=',D11.4,
     &       /5X,'ÏOPßÄOK ÄÈÔÔEPEHÖÈPOBAHÈß',15X,'P=',I1,
     &       /5X,'ØAÃ ÄÈÔÔEPEHÖÈPOBAHÈß',19X,'H=',D11.4)
  502 IF (D.EQ.0) GO TO 10002
    ...
    3 CONTINUE
      K=KK+1
C
C  H A × A Ë O   O C H O B H O Ã O   Ö È K Ë A
C
10001 K1=K1+1
      IF (.NOT.GO) GO TO 5
          IF (NG.GT.EM) C1=C/NG
...
C-------------
    ...
    5 CONTINUE
C
C         BÛ×ÈCËEHÈE BEKTOPA PACTßÆEHÈß
C
      DO 10 I=1,N
         G2(I)=0.D0
        DO 11 J=1,N
...
C
   ...
   15 CONTINUE
C
C         ÏPOBEPKA BÛÏOËHEHÈß KPÈTEPÈEB È ÏE×ATÜ
C
C-------------
      U=F(XN,FNLP)
C-------------
...
C-------------
   ...
   21 CONTINUE
      GO TO 10001
C
C  K O H E Ö    O C H O B H O Ã O   Ö È K Ë A
C
10002 IF (V.LT.S) PRINT 200
  200 FORMAT(15X,'OÁÜEM ÝËËÈÏCOÈÄA MAË')
      IF(PODRP.EQ.0) GO TO 10003
      IF (PODRP.GT.0.AND.NG.LT.E) PRINT 201
  201 FORMAT(15X,'ÄOCTÈÃHÓTA ÇAÄAHHAß TO×HOCTÜ')
      IF (PODRP.GT.0.AND.KK.EQ.D) PRINT 202
  202 FORMAT(15X,'BÛÏOËHEHO ÇAÄAHHOE ×ÈCËO ØAÃOB')
