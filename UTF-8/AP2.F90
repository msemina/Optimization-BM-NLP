...
! ОПИСАНИЕ ПЕРЕМЕННЫХ
    ...
    LOGICAL, DIMENSION(2)::PAR0,PAR1 ! ДЛИНА НЕ ВЕРНА (ВЕРОЯТНО N)
...
! ПEЧATЬ ИCXOДHЫX ДAHHЫX
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")    '     MИHИMИЗAЦИЯ METOДOM HAИCKOPEЙШEГO CПУCKA: AP2'
        ...
        WRITE(*,"(A,D12.5)")'     MAЖOPAHTA APMИЙO                     P1=',P1
        WRITE(*,"(A,D12.5)")'     KOЭФФИЦИEHT УMEHЬШEHИЯ ШAГA          P3=',P3
        WRITE(*,"(A,D12.5)")'     MAЖOPAHTA ГOЛДCTEЙHA                 P4=',P4
        WRITE(*,"(A,I5)")   '     HOMEP BEPCИИ METOДA                  V= ',L
        WRITE(*,"(A,D12.5)")'     HEOПPEДEЛEHHOCTЬ ЗAДAHИЯ ГPAHИЦ      TG=',TG
        WRITE(*,"(A,I5)")   '     ПOPЯДOK ДИФФEPEHЦИPOBAHИЯ            AC=',AC
...
END SUBROUTINE AP2
!-------------------------------------------------------------------------------------------------------------------------------
! BCПOMOГATEЛЬHAЯ ПOДПPOГPAMMA ДЛЯ AP2 ( PAHЬШE БЫЛA S )
...
END SUBROUTINE SAP2
!-------------------------------------------------------------------------------------------------------------------------------
! BCПOMOГATEЛЬHAЯ ПOДПPOГPAMMA ДЛЯ AP2 ( PAHЬШE БЫЛA F1 )
...
END FUNCTION F1AP2
!-------------------------------------------------------------------------------------------------------------------------------
! BCПOMOГATEЛЬHAЯ ПOДПPOГPAMMA ДЛЯ AP2 ( PAHЬШE БЫЛA S1 )
...
!-------------------------------------------------------------------------------------------------------------------------------
! BCПOMOГATEЛЬHAЯ ПOДПPOГPAMMA ДЛЯ AP2 ( PAHЬШE БЫЛA S2 )
