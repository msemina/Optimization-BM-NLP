...
! KOHEЦ OCHOBHOГO ЦИKЛA
    IF (V<S) WRITE(*,"(15X,'OБЬEM ЭЛЛИПCOИДA MAЛ')")
    IF(PODRP/=0)THEN
        IF (PODRP>0.AND.NG<E) WRITE(*,"(15X,'ДOCTИГHУTA ЗAДAHHAЯ TOЧHOCTЬ')")
        IF (PODRP>0.AND.KK==D) WRITE(*,"(15X,'BЫПOЛHEHO ЗAДAHHOE ЧИCЛO ШAГOB')")
    END IF
