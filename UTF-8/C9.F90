...
! 
    ...
    END IF  
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")    '     METOД ЭЛЛИПCOИДOB: C9'
        WRITE(*,"(A,D12.5)")'     TOЧHOCTЬ PEШEHИЯ ЗAДAЧИ HЛП     E= ',E
        WRITE(*,"(A,I5)")   '     ЧИCЛO ИTEPAЦИЙ                  D= ',D
        WRITE(*,"(A,D12.5)")'     PAДИУC HAЧAЛЬHOГO ШAPA          C= ',C
        WRITE(*,"(A,D12.5)")'     BEC ШTPAФA                      EC=',EC
        WRITE(*,"(A,D12.5)")'     ПAPAMETP ПPEOБP.                EE=',EE
...
! HAЧAЛO OCHOBHOГO ЦИKЛA
    ...
    DO
        ...
        IF(SP_VAR==0.OR.SP_VAR==1)THEN
            ...
            DO I=1,N
                ...
                END DO
            END DO
            ! ПPOBEPKA BЫПOЛHEHИЯ KPИTEPИEB И ПEЧATЬ
            CALL F(XN,Y1,-1)
            CALL STRFUN(L,M,Y1)
            B3=B2*B3
            B4=(B1+1)*B4
    ...
    IF(SP_VAR/=2)THEN
        SIX=6
        WRITE(*,"(10X,'ПPEД.ЧИCЛO ИTEPAЦИЙ БEЗ УЛУЧШEHИЯ')")
    END IF
...
END SUBROUTINE C9
!-------------------------------------------------------------------------------------------------------------------------------
! ПPOЦEДУPA BЫЧИCЛEHИЯ ШTPAФA И MAKCИMAЛЬHOГO OГPAHИЧEHИЯ
