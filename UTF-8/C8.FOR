...
C
C
C ПPИCBOEHИE ЗHAЧEHИЙ KOHCTAHTAM,ПAPAMETPAM METOДA
C                И ИX AHAЛИЗ.
C
      CONST1=1.D-60
      CONST2=1.D-15
      ...
      N3=N2-N1+1
C
C  BЫДEЛEHИE  AKTИBHЫX  OГPAHИЧEHИЙ
C
      POVTOR=.FALSE.
11111 CALL F(X,ZNFUNK,-1)
 ...
 3    N4=N3+MACTIV
C
C   OCHOBHЫE ДEЙCTBИЯ METOДA HЬЮTOHA
C   ЗAДAHИE ЗHAЧEHИЙ  XPR,DVOY
C
      N11=N1-1
 ...
 77   CONTINUE
      ...
      PRINT 222
  222 FORMAT(/46X,'METOД HЬЮTOHA : C8')
      PRINT 555,E,D,MAJGOS,SLOY,SHAGD,AA,SHAGG,AC
  555 FORMAT(/5X,'TOЧHOCTЬ PEШEHИЯ ЗAДAЧИ HЛП',6X,'E=',D12.5,
     &       /5X,'ЧИCЛO ИTEPAЦИЙ',19X,'D=',I5,
     &       /5X,'MAЖOPAHTA ГOЛДCTEЙHA',13X,'MG=',D12.5,
     &       /5X,'ПAPAMETP ДЛЯ BЫДEЛ.AKTИBHЫX OГP.',1X,'SLOY=',D12.5,
     &       /5X,'ШAГ ДИФФEPEHЦИPOBAHИЯ',12X,'SHAGD=',D12.5,
     &       /5X,'ПOPЯДOK ДИФФEPEHЦИPOBAHИЯ',8X,'AA=',I5,
     &       /5X,'ШAГ BЫЧИCЛEHИЯ ГECCИAHA',10X,'SHAGG=',D12.5,
     &       /5X,'ПOPЯДOK BЫЧИCЛEHИЯ ГECCИAHA',6X,'AC=',I5)
  333 IF(.NOT.(POVTOR))
     &               CALL PRTNLP(0,N,L,M,X,ZNFUNK,P,ST,CRIT,1,4,TEXT)
C
C  O C H O B H O Й    Ц И K Л    П O    K
C
33333 K=KK+1
      IF(.NOT.(K.LT.D+1.AND.DABS(CRIT).GT.E)) GO TO 22222
C
C  BЫЧИCЛEHИE И OБPAЩEHИE ГECCИAHA LZZ
C
 ...
 131  CALL DMINV(LZZ,N4,DET,LRAB,MRAB)
      IF(DET.NE.0) GO TO 161
         PRINT 162
 162     FORMAT(/10X,'ГECCИAH BЫPOЖДEH')
         RETURN
C
C  BЫЧИCЛEHИE  HЬЮTOHOBCKOГO  HAПPABЛEHИЯ
C
 161  DO 16 I=1,N4
         NAPR(I)=0
         DO 17 I1=1,N4
 ...
 16   CONTINUE
C
C  PEГУЛИPOBKA ДЛИHЫ ШAГA BДOЛЬ HAПPABЛEHИЯ
C
      STEP=1
 ...
 211  CALL F(XPR,ZNFPR,-1)
     ...
     &                    GO TO 19
      ...
      GO TO 18
C
C  ПPOBEPИM,OБHУЛИЛACЬ ЛИ KAKAЯ-ЛИБO ДBOЙCTBEHHAЯ ПEPEMEHHAЯ
C           ПPИ HAPУШEHHOM AKTИBHOM OГPAHИЧEHИИ
C
 ...
 23         P(I1)=10*NACHDV
 25         PRINT 26
 26         FORMAT(/10X,'METOД CXOДИTCЯ HE K PEШEHИЮ')
            RETURN
 22      CONTINUE
C
C  ПPOBEPИM, ДOБABИЛИCЬ ЛИ HOBЫE AKTИBHЫE OГPAHИЧEHИЯ
C
 ...
 27   CONTINUE
C
C  ПEPEKAЧKA ИЗ MACCИBOB: XPR,DVPR,ZNFPR,NORLZ1 B MACCИBЫ:
...
C
 ...
 9    CONTINUE
      ...
      END
C
C  B Ы Ч И C Л E H И E    Ш T P A Ф A
C
      SUBROUTINE SHTRAF
