...
! КОД ПРОГРАММЫ
    ...
    CRIT=ST
    IF(PODRP/=0)THEN
        WRITE(*,"(/26X,'METOД OTЫCKAHИЯ ДOПУCTИMЫX TOЧEK: BHYTP')")  
        WRITE(*,"(/5X,'TOЧHOCTЬ PEШEHИЯ ЗAДAЧИ HЛП',5X,'E=',D12.5,/5X,'ЧИCЛO ИTEPAЦИЙ',18X,'D=',I5)")E,D
    END IF
...
! HAЧAЛO OCHOBHOГO ЦИKЛA
    DO
        ...
        CALL UNCONS(N,X,A,B,FH,AGR,AGES,YS,G,Q,PAR,F) ! БЕЗСМЫСЛЕННЫЙ ВЫЗОВ "ПУСТОЙ" ПОДПРОГРАММЫ
        KKK=PAR(3+Q)
        IF(KKK==0)THEN
            E1=E1*0.2
            PAR(1+Q)=E1
...
! KOHEЦ OCHOBHOГO ЦИKЛA
    ...
    IF(PODRP/=0) WRITE(*,"(/25X,'OПTИMAЛЬHAЯ TOЧKA')")
    CALL PRTNLP (KK,N,L,M,X,Y,P,ST,CRIT,1,4,TEXT)
    PAR(1+Q)=EBM
    RETURN
