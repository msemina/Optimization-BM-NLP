! ДВОЙСТВЕННЫЙ МЕТОД МОДИФИЦИРОВАННОЙ ФУНКЦИИ ЛАГРАНЖА
...
! 
    ...
    END DO
    IF(PODRP/=0)THEN
        WRITE(*,"(/A/)")    '     ДBOЙCTBEHHЫЙ METOД MOДИФИЦИPOBAHHOЙ ФУHKЦИИ ЛAГPAHЖA: CP5'
        ...
        WRITE(*,"(A,D12.5)")'     ПAPAMETP PEГУЛЯPИЗAЦИИ          R=  ',R
        WRITE(*,"(A,D12.5)")'     ПAPAMETP BCПOMOГ. ЗAДAЧИ        ES= ',ES
        WRITE(*,"(A,D12.5)")'     BECOBOЙ KOЭФФИЦИEHT             R1= ',R1
    END IF
    ...
    IF(L1<=M)THEN
        DO I=L1,M
            IF(P(I)<-EW)THEN
                WRITE(*,"(/10X,'ДBOЙCTBEHЫЙ BEKTOP HEДOПУCTИM')")
                IF(PODRP/=0)WRITE(*,"(/32X,'OПTИMAЛЬHAЯ TOЧKA')")
                PAR(1+Q)=EBM
                CALL PRTNLP(KK,N,L,M,X,Y,P,ST,CRIT,1,4,TEXT)
                RETURN
