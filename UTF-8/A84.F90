...
! ПEЧATЬ ИCXOДHЫX ДAHHЫX
    IF(PODRP/=0)THEN
        WRITE(*,"(5X,'MИHИMИЗAЦИЯ KBAЗИHЬЮTOHOBCKИM METOДOM')")
        ...
        WRITE(*,"(5X,'HOMEP BEPCИИ METOДA',18X,               'V=',I3)")V
        WRITE(*,"(5X,'ПOPЯДOK ДИФФEPEHЦИPOBAHИЯ',12X,         'AC=',I3)")PORDIF
        ...
        IF(SP_VAR==300)THEN
            DO
                ...
                IF(DABS(C3)<DABS(C0))THEN
                    IF(GRAT)THEN
                        WRITE(*,"(5X,'OДHOMEPHЫЙ ПOИCK HEУДAЧEH')")
                        EXIT
                    END IF
                    GRAT=.TRUE.
                    SP_VAR=100
